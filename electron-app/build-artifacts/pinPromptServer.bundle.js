// electron-app/main/pinPromptServer.js
var { BrowserWindow, ipcMain } = require("electron");
var http = require("http");
var pinPrompt = { server: null, port: null, token: null, ready: null, busy: false, win: null };
  ...
  return new Promise((resolve) => {
    try {
      if (pinPrompt.win && !pinPrompt.win.isDestroyed()) {
        ...
        const cleanup = () => {
          ipcMain.removeListener("pin:submit", onSubmit2);
          ipcMain.removeListener("pin:cancel", onCancel2);
          try {
            if (w2 && !w2.isDestroyed()) {
              w2.removeListener("closed", onClosed);
            }
          } catch {
          }
          try {
        ...
        ipcMain.once("pin:submit", onSubmit2);
        ipcMain.once("pin:cancel", onCancel2);
        try {
          w2.once("closed", onClosed);
        } catch {
        }
        return;
      }
      const w = new BrowserWindow({
        ...
        modal: false,
        skipTaskbar: false,
        parent: (typeof getMainWindow === "function" ? getMainWindow() : null) || void 0,
        show: false,
    ...
    const srv = http.createServer(async (req, res) => {
      try {
        if (req.method === "POST" && req.url === "/prompt-pin") {
          ...
          const pin = await showPinDialog(hint, getMainWindow).catch(() => void 0);
          try {
            if (!pin && pin !== "") {
