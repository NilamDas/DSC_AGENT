...
var { app, BrowserWindow, Tray, Menu, ipcMain, dialog, nativeImage } = require("electron");
var path = require("path");
var fs = require("fs");
var { spawn } = require("child_process");
var { ensureReady: ensurePinPromptServerReady } = require("./pinPromptServer.loader.js");
...
var pendingControlPanelShow = false;
...
var LOG = (msg) => {
  ...
  console.log(line);
  lastLogs.push(line);
  if (lastLogs.length > 2e3) lastLogs.shift();
};
...
async function startAgent() {
  ...
  try {
    ...
    agentProc.on("exit", (code, signal) => {
      ...
      try {
        showTrayNotification("DSC Agent", "Agent Stopped");
      } catch (e) {
      }
      agentProc = null;
      updateTrayMenu();
    ...
    if (agentProc) {
      if (process.platform === "win32") {
        try {
          ...
          spawn2("taskkill", ["/PID", String(pid), "/T", "/F"], { windowsHide: true });
        } catch {
        }
      } else {
        try {
  ...
  if (!desired) {
    if ((process.platform === "darwin" || process.platform === "win32") && app.isPackaged) {
      try {
        ...
        LOG(`Failed to disable auto-launch: ${err.message || err}`);
      }
    }
    return;
  }
...
async function fetchJsonOrError(url) {
  try {
    ...
    if (!res.ok) {
      const body = typeof parsed === "object" ? JSON.stringify(parsed, null, 2) : String(parsed);
      throw new Error(
          ...
          submenu: [
            {
              ...
              click: async () => {
                ...
                dialog.showMessageBox({
                  ...
                  message: data.ok ? JSON.stringify(data.data, null, 2) : data.error
                });
              }
            },
            {
...
ipcMain.handle("settings:set", (evt, s) => {
  saveSettings(s || {});
  return { ok: true };
});
...
ipcMain.handle("notify", (evt, title, body) => {
  try {
    showTrayNotification(title || "DSC Agent", body || "");
  } catch (e) {
    LOG(`notify failed: ${e && e.message ? e.message : e}`);
  }
  return { ok: true };
});
