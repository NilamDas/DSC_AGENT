...
app.post('/sign/pdf', requireAuth, async (req, res) => {
  try {
    ...
    // If a session PIN exists and no explicit pin provided, use it
    if (!pin && SESSION_PIN) pin = SESSION_PIN;
    let requirePin = (req.body && req.body.requirePin === true) || REQUIRE_PIN_PER_SIGN;
    // If we already have a session PIN, do not prompt again unless explicitly forced by requirePin=true
    ...
    if (requirePin) {
      try {
        ...
        // If client asked to remember for this session, cache the PIN in-memory (process lifetime only)
    ...
    // Client may explicitly clear any remembered PIN for this session
    ...
    // New: only duplicate clickable widgets across pages when explicitly requested
    const duplicateWidgets = !!(req.body && req.body.duplicateWidgets === true);
    // Per-request placement controls
    ...
    const rectMode = (req.body && typeof req.body.rectMode === 'string') ? String(req.body.rectMode).toLowerCase() : 'pdf';

    const inputBuf = Buffer.from(b64, 'base64');
    assertNoExistingSignature(inputBuf);

    // Proceed with original single-PDF flow below (kept stable)

    // Default name for early stamping before we read the token cert
    let stampUserName = 'Unknown';

    // Ask viewers to generate a visible appearance for form fields when needed
    // by setting /NeedAppearances true. This keeps placeholder visible even if
    // custom AP injection fails (e.g., edge-case PDFs).
    let pdfForPlaceholder = inputBuf;
    try {
      const pdfDoc = await PDFDocument.load(inputBuf);
      // Enable viewer-generated appearances as a safe default; when we inject a
      // custom AP below, viewers will still respect the explicit appearance.
      pdfDoc.catalog.set(PDFName.of('NeedAppearances'), pdfDoc.context.obj(true));

      pdfForPlaceholder = await pdfDoc.save({ useObjectStreams: false });
    } catch (e) {
      console.warn('Setting NeedAppearances or stamping text failed:', e.message || e);
    }
    ...
    const intermediates = await fetchIntermediatesIfRequested(signerCert, embedIntermediates);
    const userName = signerCert.subject.typesAndValues.find(tv => tv.type === '2.5.4.3')?.value.valueBlock.value || 'Unknown';
    stampUserName = userName;

    // --- Step 1: Use pdf-lib to set the /NeedAppearances flag ---
//    const pdfDoc = await PDFDocument.load(inputBuf);
//    // This flag tells the PDF viewer that it is responsible for generating appearance streams.
//    pdfDoc.catalog.set(PDFName.of('NeedAppearances'), PDFBoolean.True);
//    const pdfBytesWithFlag = await pdfDoc.save({ useObjectStreams: false });
//    // --- End of pdf-lib modification ---

    // Optional: stamp all pages except the target page (to avoid double-bold when AP is drawn there)
    if (stampAllPages) {
      try {
        // Compute a concrete rectangle for stamping, honoring rectMode 'top-left' and 2-value [left, top]
        let stampRect = rectOverride || SIGN_RECT;
        try {
          const pdfDocTmp = await PDFDocument.load(pdfForPlaceholder);
          const pageCountTmp = pdfDocTmp.getPageCount();
          const targetIndexTmp = (requestedPageIndex === 'last') ? (pageCountTmp - 1)
            : (requestedPageIndex && requestedPageIndex > 0 ? Math.min(pageCountTmp - 1, requestedPageIndex - 1) : 0);
          const sz = pdfDocTmp.getPages()[targetIndexTmp].getSize();
          const rectMode = (req.body && req.body.rectMode) || 'pdf';
          if (rectOverride && rectMode === 'top-left') {
            const arr = rectOverride.map((n) => parseInt(n, 10));
            const left = arr[0] || 0; const top = arr[1] || 0;
            const width = (arr.length >= 3) ? Math.max(0, arr[2]) : Math.max(1, Math.floor(sz.width - Math.max(0, left)));
            const height = (arr.length >= 4) ? Math.max(0, arr[3]) : Math.max(1, Math.floor(sz.height - Math.max(0, top)));
            const x1 = Math.max(0, left);
            const y2 = Math.max(0, sz.height - top);
            const x2 = Math.min(sz.width, x1 + width);
            const y1 = Math.max(0, y2 - height);
        ...
        pdfForPlaceholder = await applyTextStampToAllPages(
          pdfForPlaceholder,
          userName,
          signingTime,
          stampRect,
          anchor,
          requestedPageIndex // skip this page; AP will serve as visible mark here
        );
      } catch (e) { console.warn('Stamp all pages failed:', e.message || e); }
    }

    // --- Step 2: Create the placeholder using the modified PDF ---
    // Prefer pdf-lib placeholder so we can inject a visible AP (appearance stream)
    const widgetRect = rectOverride || SIGN_RECT;
    ...
    try {
      ...
      // Inject a visible appearance stream into the signature widget (non-image, vector text)
      try {
      const context = pdfDoc2.context;
      const acroForm = pdfDoc2.catalog.lookup(PDFName.of('AcroForm'));
      const fields = acroForm.lookup(PDFName.of('Fields'));
      ...
      // Resolve widget annotation from field's Kids (first kid)
      let widgetRef = null;
      let widget = null;
      try {
        const kids = field.lookup(PDFName.of('Kids'));
        widgetRef = kids && (kids.get ? kids.get(0) : (kids.array ? kids.array[0] : null));
        if (widgetRef) widget = context.lookup(widgetRef);
      } catch {}
      if (!widget) { widgetRef = fieldRef; widget = field; }
        // Determine target page (for top-left computations and safety)
        const pageCount = pdfDoc2.getPageCount();
        const targetIndex = (requestedPageIndex === 'last') ? (pageCount - 1)
          : (requestedPageIndex && requestedPageIndex > 0 ? Math.min(pageCount - 1, requestedPageIndex - 1) : 0);
        const targetPage = pdfDoc2.getPages()[targetIndex];
        // Read widget rect safely, fall back to chosen widgetRect
        let x1, y1, x2, y2;
        try {
          const rectArr = widget.lookup(PDFName.of('Rect'));
          ...
          x1 = widgetRect[0]; y1 = widgetRect[1]; x2 = widgetRect[2]; y2 = widgetRect[3];
        }
        // If rect override provided, use it (ensuring proper ordering)
            if (rectOverride) {
              if (rectMode === 'top-left') {
                ...
                const defW = Math.max(1, Math.min(200, Math.floor(size.width - Math.max(0, left))));
                const defH = Math.max(1, Math.min(50, Math.floor(size.height - Math.max(0, top))));
                const Wd = (arr.length >= 3 && Number.isFinite(arr[2])) ? Math.max(1, arr[2]) : defW;
                const Ht = (arr.length >= 4 && Number.isFinite(arr[3])) ? Math.max(1, arr[3]) : defH;
                x1 = Math.max(0, left);
                const y2tl = Math.max(0, size.height - top);
                x2 = Math.min(size.width, x1 + Wd);
                y1 = Math.max(0, y2tl - Ht);
                y2 = y2tl;
              } else {
                const [rx1, ry1, rx2, ry2] = rectOverride;
        ...
        // Ensure sane widget rect; if zero/negative, expand to default box
        const pageSizeFresh = targetPage.getSize();
        if (!Number.isFinite(x1) || !Number.isFinite(y1) || !Number.isFinite(x2) || !Number.isFinite(y2)) {
          x1 = 36; y1 = 36; x2 = 236; y2 = 86;
          try { widget.set(PDFName.of('Rect'), context.obj([x1, y1, x2, y2])); } catch {}
        }
        if (x2 <= x1 || y2 <= y1) {
          x2 = Math.min(pageSizeFresh.width, x1 + 200);
          y2 = Math.min(pageSizeFresh.height, y1 + 50);
          try { widget.set(PDFName.of('Rect'), context.obj([x1, y1, x2, y2])); } catch {}
        }
        const wMax = Math.max(0, x2 - x1);
        const hMax = Math.max(0, y2 - y1);
        ...
        const w = Math.min(wMax, Math.ceil(contentW) + padX*2);
        const h = Math.min(hMax, (2*fontSize) + (lh - fontSize) + padY*2);
        // Shrink widget rect to match the stamp box, honoring anchor
        let newX1 = x1, newY1 = y1, newX2 = x2, newY2 = y2;
        switch (anchor) {
          case 'top-right':
            newX2 = x2; newX1 = newX2 - w; newY2 = y2; newY1 = newY2 - h; break;
        ...
        // Ensure viewers show AP immediately
        try {
          const acro = pdfDoc2.catalog.lookup(PDFName.of('AcroForm'));
          acro.set(PDFName.of('SigFlags'), PDFNumber.of(3));
          if (pdfDoc2.catalog.delete) pdfDoc2.catalog.delete(PDFName.of('NeedAppearances'));
        } catch {}

        // Also draw visible text into page content at the widget rect for first-open visibility
        try {
          const yTopDraw = Math.max(0, newY1 + (newY2 - newY1) - padY - fontSize);
          const pageDraw = targetPage;
          // background for contrast
          try { pageDraw.drawRectangle({ x: newX1, y: newY1, width: Math.max(1, w), height: Math.max(1, h), color: rgb(1,1,1), opacity: 1, borderOpacity: 0 }); } catch {}
          try { pageDraw.drawText(line1Fit, { x: newX1 + padX, y: yTopDraw, size: fontSize, font: helv, color: rgb(0,0,0) }); } catch {}
          try { pageDraw.drawText(line2Fit, { x: newX1 + padX, y: Math.max(0, yTopDraw - lh), size: fontSize, font: helv, color: rgb(0,0,0) }); } catch {}
        } catch {}

        // Reposition existing widget to the target page and ensure it's listed in Annots
        try {
          const pageCount = pdfDoc2.getPageCount();
          ...
          const targetPage = pdfDoc2.getPages()[targetIndex];
          try { widget.set(PDFName.of('P'), targetPage.ref); } catch {}
          try { targetPage.node.set(PDFName.of('Annots'), context.obj([ widgetRef ])); } catch {}
        } catch {}

        // Reposition widget to requested page if provided (only when multi-page)
        try {
          const pageCount = pdfDoc2.getPageCount();
          if (requestedPageIndex !== null && requestedPageIndex !== undefined && pageCount > 1) {
            const targetIndex = (requestedPageIndex === 'last') ? (pageCount - 1) : (Math.max(0, Math.min(pageCount - 1, (requestedPageIndex > 0 ? requestedPageIndex - 1 : 0))));
            const targetPage = pdfDoc2.getPages()[targetIndex];
            // Update widget's page pointer
            widget.set(PDFName.of('P'), targetPage.ref);
            // Remove from any existing page Annots arrays, then add to target
            for (let i = 0; i < pageCount; i++) {
              ...
              try {
                ...
                for (let j = size - 1; j >= 0; j--) {
                  ...
                  if (ref && ref === widgetRef && annots.remove) annots.remove(j);
                }
              } catch {}
            }
            try {
              // Force Annots array to contain our widget explicitly
              targetPage.node.set(PDFName.of('Annots'), context.obj([ widgetRef ]));
            } catch {}
            // Duplicate signature widgets on other pages (optional)
            try {
              if (duplicateWidgets) {
                const pageCount2 = pdfDoc2.getPageCount();
                const targetIndex2 = (requestedPageIndex === 'last') ? (pageCount2 - 1)
                  : (requestedPageIndex && requestedPageIndex > 0 ? Math.min(pageCount2 - 1, requestedPageIndex - 1) : 0);
                // Parent field dictionary
                let fieldRef = null;
                try {
                  const parentMaybe = widget.lookupMaybe ? widget.lookupMaybe(PDFName.of('Parent')) : null;
                  if (parentMaybe) fieldRef = parentMaybe.ref || null;
                } catch {}
                if (!fieldRef) {
                  const kidsArr = context.obj([ lastFieldRef ]);
                  const fieldDict = context.obj({ FT: PDFName.of('Sig'), Kids: kidsArr, T: PDFString.of('Signature1') });
                  try { const vMaybe = widget.lookupMaybe ? widget.lookupMaybe(PDFName.of('V')) : null; if (vMaybe) { fieldDict.set(PDFName.of('V'), vMaybe); try { widget.delete(PDFName.of('V')); } catch {} } } catch {}
                  fieldRef = context.register(fieldDict);
                  try { widget.set(PDFName.of('Parent'), fieldRef); } catch {}
                  try {
                    if (fields) {
                      const size = fields.size ? fields.size() : (fields.array ? fields.array.length : 0);
                      for (let i = size - 1; i >= 0; i--) {
                        const ref = fields.get ? fields.get(i) : (fields.array ? fields.array[i] : null);
                        if (ref && ref === lastFieldRef && fields.remove) fields.remove(i);
                      }
                      if (fields.push) fields.push(fieldRef);
                    }
                  } catch {}
                  try { const acroForm = pdfDoc2.catalog.lookup(PDFName.of('AcroForm')); acroForm.set(PDFName.of('SigFlags'), PDFNumber.of(3)); } catch {}
                } else {
                  try {
                    const parentDict = context.lookup(fieldRef);
                    const vMaybe = widget.lookupMaybe ? widget.lookupMaybe(PDFName.of('V')) : null;
                    if (vMaybe) { parentDict.set(PDFName.of('V'), vMaybe); try { widget.delete(PDFName.of('V')); } catch {} }
                    if (fields) {
                      const size = fields.size ? fields.size() : (fields.array ? fields.array.length : 0);
                      for (let i = size - 1; i >= 0; i--) {
                        const ref = fields.get ? fields.get(i) : (fields.array ? fields.array[i] : null);
                      ...
                      let has = false;
                      try {
                        const size2 = fields.size ? fields.size() : (fields.array ? fields.array.length : 0);
                        for (let i = 0; i < size2; i++) { const r = fields.get ? fields.get(i) : fields.array[i]; if (r === fieldRef) { has = true; break; } }
                      } catch {}
                      if (!has && fields.push) fields.push(fieldRef);
                    }
                  } catch {}
                  try { const acroForm = pdfDoc2.catalog.lookup(PDFName.of('AcroForm')); acroForm.set(PDFName.of('SigFlags'), PDFNumber.of(3)); } catch {}
                }
                for (let i2 = 0; i2 < pageCount2; i2++) {
                  if (i2 === targetIndex2) continue;
                  const pg2 = pdfDoc2.getPages()[i2];
                  const maxTextW2 = Math.max(0, (Math.max(0, x2 - x1) - padX*2));
                  const line1Fit2 = fitText(line1, maxTextW2);
                  const line2Fit2 = fitText(line2, maxTextW2);
                  const contentW2 = Math.max(helv.widthOfTextAtSize(line1Fit2, fontSize), helv.widthOfTextAtSize(line2Fit2, fontSize));
                  const w2 = Math.min(Math.max(0, x2 - x1), Math.ceil(contentW2) + padX*2);
                  const h2 = Math.min(Math.max(0, y2 - y1), (2*fontSize) + (lh - fontSize) + padY*2);
                  let X1 = x1, Y1 = y1, X2 = x2, Y2 = y2;
                  switch (anchor) { case 'top-right': X2 = x2; X1 = X2 - w2; Y2 = y2; Y1 = Y2 - h2; break; case 'bottom-left': X1 = x1; X2 = X1 + w2; Y1 = y1; Y2 = Y1 + h2; break; case 'bottom-right': X2 = x2; X1 = X2 - w2; Y1 = y1; Y2 = Y1 + h2; break; case 'top-left': default: X1 = x1; X2 = X1 + w2; Y2 = y2; Y1 = Y2 - h2; break; }
                  const content2 = ['BT', `/F1 ${fontSize} Tf`, `1 0 0 1 ${padX} ${Math.max(0, h2 - padY - fontSize)} Tm`, `(${esc(line1Fit2)}) Tj`, `1 0 0 1 ${padX} ${Math.max(0, h2 - padY - fontSize - lh)} Tm`, `(${esc(line2Fit2)}) Tj`, 'ET'].join('\n');
                  const ap2 = context.stream(content2, { Type:'XObject', Subtype:'Form', BBox:[0,0, w2, h2], Matrix:[1,0,0,1,0,0], Resources:{ Font:{ F1: helv.ref } } });
                  const newWidget = context.obj({ Type:'Annot', Subtype:'Widget', Rect:[X1, Y1, X2, Y2], P: pg2.ref, AP:{ N: context.register(ap2) }, F: 4, Parent: fieldRef });
                  const newWidgetRef = context.register(newWidget);
                  try { let ann2 = pg2.node.lookupMaybe ? pg2.node.lookupMaybe(PDFName.of('Annots')) : null; if (!ann2) pg2.node.set(PDFName.of('Annots'), context.obj([ newWidgetRef ])); else if (ann2.push) ann2.push(newWidgetRef); } catch {}
                  try { const parentDict = context.lookup(fieldRef); const kids = parentDict.lookupMaybe ? parentDict.lookupMaybe(PDFName.of('Kids')) : null; if (kids && kids.push) kids.push(newWidgetRef); } catch {}
                }
              }
            } catch {}
          }
        ...
        // Duplicate signature widgets on other pages so clicking opens signature properties (optional)
        try {
          if (duplicateWidgets) {
            const pageCount2 = pdfDoc2.getPageCount();
            const targetIndex2 = (requestedPageIndex === 'last') ? (pageCount2 - 1)
              : (requestedPageIndex && requestedPageIndex > 0 ? Math.min(pageCount2 - 1, requestedPageIndex - 1) : 0);
            // Create or resolve parent field dictionary
            let fieldRef = null;
            try {
              const parentMaybe = widget.lookupMaybe ? widget.lookupMaybe(PDFName.of('Parent')) : null;
              if (parentMaybe) {
                fieldRef = parentMaybe.ref || null;
              }
            } catch {}
            if (!fieldRef) {
              const kidsArr = context.obj([ lastFieldRef ]);
              const fieldDict = context.obj({ FT: PDFName.of('Sig'), Kids: kidsArr, T: PDFString.of('Signature1') });
              // Also mirror the signature dictionary (/V) at the field level so all widgets see it
              try {
                const vMaybe = widget.lookupMaybe ? widget.lookupMaybe(PDFName.of('V')) : null;
                if (vMaybe) { fieldDict.set(PDFName.of('V'), vMaybe); try { widget.delete(PDFName.of('V')); } catch {} }
              } catch {}
              fieldRef = context.register(fieldDict);
              try { widget.set(PDFName.of('Parent'), fieldRef); } catch {}
              try {
                if (fields) {
                  // Remove old widget ref and ensure field ref is present
                  const size = fields.size ? fields.size() : (fields.array ? fields.array.length : 0);
                  for (let i = size - 1; i >= 0; i--) {
                    const ref = fields.get ? fields.get(i) : (fields.array ? fields.array[i] : null);
                    if (ref && ref === lastFieldRef && fields.remove) fields.remove(i);
              ...
              // Ensure parent has /V pointing to the same signature dict
              try {
                const parentDict = context.lookup(fieldRef);
                const vMaybe = widget.lookupMaybe ? widget.lookupMaybe(PDFName.of('V')) : null;
                if (vMaybe) { parentDict.set(PDFName.of('V'), vMaybe); try { widget.delete(PDFName.of('V')); } catch {} }
                // Ensure parent is in Fields and old widget ref removed
                if (fields) {
                  const size = fields.size ? fields.size() : (fields.array ? fields.array.length : 0);
                  for (let i = size - 1; i >= 0; i--) {
                    const ref = fields.get ? fields.get(i) : (fields.array ? fields.array[i] : null);
            ...
            // Build widgets for other pages
            for (let i = 0; i < pageCount2; i++) {
              if (i === targetIndex2) continue;
              const pg = pdfDoc2.getPages()[i];
              // Compute rect per anchor
              const maxTextW2 = Math.max(0, (Math.max(0, x2 - x1) - padX*2));
              const line1Fit2 = fitText(line1, maxTextW2);
              const line2Fit2 = fitText(line2, maxTextW2);
              const contentW2 = Math.max(helv.widthOfTextAtSize(line1Fit2, fontSize), helv.widthOfTextAtSize(line2Fit2, fontSize));
              ...
              let X1 = x1, Y1 = y1, X2 = x2, Y2 = y2;
              switch (anchor) {
                case 'top-right': X2 = x2; X1 = X2 - w2; Y2 = y2; Y1 = Y2 - h2; break;
                case 'bottom-left': X1 = x1; X2 = X1 + w2; Y1 = y1; Y2 = Y1 + h2; break;
                case 'bottom-right': X2 = x2; X1 = X2 - w2; Y1 = y1; Y2 = Y1 + h2; break;
                case 'top-left': default: X1 = x1; X2 = X1 + w2; Y2 = y2; Y1 = Y2 - h2; break;
              }
              const content2 = [
                'BT',
                `/F1 ${fontSize} Tf`,
                `1 0 0 1 ${padX} ${Math.max(0, h2 - padY - fontSize)} Tm`,
                `(${esc(line1Fit2)}) Tj`,
                `1 0 0 1 ${padX} ${Math.max(0, h2 - padY - fontSize - lh)} Tm`,
                `(${esc(line2Fit2)}) Tj`,
                'ET',
              ].join('\n');
              const ap2 = context.stream(content2, { Type:'XObject', Subtype:'Form', BBox:[0,0, w2, h2], Matrix:[1,0,0,1,0,0], Resources:{ Font:{ F1: helv.ref } } });
              const newWidget = context.obj({
                Type: 'Annot', Subtype: 'Widget',
                Rect: [X1, Y1, X2, Y2],
                P: pg.ref,
                AP: { N: context.register(ap2) },
                F: 4,
                Parent: fieldRef,
              });
              const newWidgetRef = context.register(newWidget);
              try {
                let ann = pg.node.lookupMaybe ? pg.node.lookupMaybe(PDFName.of('Annots')) : null;
                if (!ann) pg.node.set(PDFName.of('Annots'), context.obj([ newWidgetRef ])); else if (ann.push) ann.push(newWidgetRef);
              } catch {}
              try {
                const parentDict = context.lookup(fieldRef);
                const kids = parentDict.lookupMaybe ? parentDict.lookupMaybe(PDFName.of('Kids')) : null;
                if (kids && kids.push) kids.push(newWidgetRef);
              } catch {}
            }
          }
        } catch {}
      } catch (apErr) {
        console.warn('Failed to build AP stream:', apErr.message || apErr);
      }

      pdfWithPlaceholder = await pdfDoc2.save({ useObjectStreams: false });
    } catch (eLib) {
      console.warn('pdf-lib placeholder failed, switching to plain placeholder:', eLib.message || eLib);
      pdfWithPlaceholder = plainAddPlaceholder({
        pdfBuffer: Buffer.from(pdfForPlaceholder),
        reason,
        name: userName,
        signatureLength: 32768,
        widgetRect, // [x1, y1, x2, y2]
      });
      // Best-effort: inject visible AP and optionally duplicate widgets across pages even when falling back
      try {
        const pdfDoc3 = await PDFDocument.load(pdfWithPlaceholder);
        try { pdfDoc3.catalog.set(PDFName.of('NeedAppearances'), pdfDoc3.context.obj(true)); } catch {}
        const context = pdfDoc3.context;
        const acroForm = pdfDoc3.catalog.lookup(PDFName.of('AcroForm'));
        const fields = acroForm && acroForm.lookup ? acroForm.lookup(PDFName.of('Fields')) : null;
        if (fields && fields.size && fields.size() >= 1) {
          const fieldRef = fields.get(fields.size() - 1);
          const field = context.lookup(fieldRef);
          ...
          if (!widget) { widgetRef = fieldRef; widget = field; }
          // Read widget rect safely (fallback to widgetRect)
          let x1, y1, x2, y2;
          try {
            const rectArr = widget.lookup(PDFName.of('Rect'));
            x1 = rectArr.get(0).asNumber();
          ...
          const wMax = Math.max(0, x2 - x1);
          const hMax = Math.max(0, y2 - y1);
          const helv = await pdfDoc3.embedFont(StandardFonts.Helvetica);
          const pad = (n) => String(n).padStart(2, '0');
          const dt = signingTime || new Date();
          const tsText = `${dt.getFullYear()}-${pad(dt.getMonth()+1)}-${pad(dt.getDate())} ${pad(dt.getHours())}:${pad(dt.getMinutes())}:${pad(dt.getSeconds())}`;
          const line1 = `Digitally signed by ${userName}`;
          ...
          const lh = 14;
          const padX = 6, padY = 6;
          const fitText = (t, maxW) => { let s = String(t); while (helv.widthOfTextAtSize(s, fontSize) > maxW && s.length > 1) s = s.slice(0, -1); if (helv.widthOfTextAtSize(s, fontSize) > maxW && s.length > 1) s = s.slice(0, -1); return s; };
          ...
          const h = Math.min(hMax, (2*fontSize) + (lh - fontSize) + padY*2);
          const content = ['BT', `/F1 ${fontSize} Tf`, `1 0 0 1 ${padX} ${Math.max(0, h - padY - fontSize)} Tm`, `(${esc(line1Fit)}) Tj`, `1 0 0 1 ${padX} ${Math.max(0, h - padY - fontSize - lh)} Tm`, `(${esc(line2Fit)}) Tj`, 'ET'].join('\n');
          const ap = context.stream(content, { Type:'XObject', Subtype:'Form', BBox:[0,0, w, h], Matrix:[1,0,0,1,0,0], Resources:{ Font:{ F1: helv.ref } } });
          try { widget.set(PDFName.of('AP'), context.obj({ N: context.register(ap) })); } catch {}
          // Reposition existing widget to target page and ensure it is listed
          try {
            const pageCount2 = pdfDoc3.getPageCount();
            const targetIndex2 = (requestedPageIndex === 'last') ? (pageCount2 - 1)
              : (requestedPageIndex && requestedPageIndex > 0 ? Math.min(pageCount2 - 1, requestedPageIndex - 1) : 0);
            const targetPage2 = pdfDoc3.getPages()[targetIndex2];
            try { widget.set(PDFName.of('P'), targetPage2.ref); } catch {}
            try { targetPage2.node.set(PDFName.of('Annots'), context.obj([ widgetRef ])); } catch {}
          } catch {}
          try { const acro = pdfDoc3.catalog.lookup(PDFName.of('AcroForm')); acro.set(PDFName.of('SigFlags'), PDFNumber.of(3)); } catch {}
          ...
          if (duplicateWidgets) {
            ...
            const targetIndex2 = (requestedPageIndex === 'last') ? (pageCount2 - 1)
              ...
              try { const parentMaybe = widget.lookupMaybe ? widget.lookupMaybe(PDFName.of('Parent')) : null; if (parentMaybe) parentFieldRef = parentMaybe.ref || null; } catch {}
              if (!parentFieldRef) {
                const kidsArr = context.obj([ widgetRef ]);
                const fieldDict = context.obj({ FT: PDFName.of('Sig'), Kids: kidsArr, T: PDFString.of('Signature1') });
                parentFieldRef = context.register(fieldDict);
                try { widget.set(PDFName.of('Parent'), parentFieldRef); } catch {}
                try { if (fields && fields.push) fields.push(parentFieldRef); } catch {}
                try { const acro = pdfDoc3.catalog.lookup(PDFName.of('AcroForm')); acro.set(PDFName.of('SigFlags'), PDFNumber.of(3)); } catch {}
              }
              for (let i2 = 0; i2 < pageCount2; i2++) {
                if (i2 === targetIndex2) continue;
                const pg2 = pdfDoc3.getPages()[i2];
              const maxTextW2 = Math.max(0, (Math.max(0, x2 - x1) - padX*2));
              const line1Fit2 = fitText(line1, maxTextW2);
              const line2Fit2 = fitText(line2, maxTextW2);
              const contentW2 = Math.max(helv.widthOfTextAtSize(line1Fit2, fontSize), helv.widthOfTextAtSize(line2Fit2, fontSize));
              ...
              const content2 = ['BT', `/F1 ${fontSize} Tf`, `1 0 0 1 ${padX} ${Math.max(0, h2 - padY - fontSize)} Tm`, `(${esc(line1Fit2)}) Tj`, `1 0 0 1 ${padX} ${Math.max(0, h2 - padY - fontSize - lh)} Tm`, `(${esc(line2Fit2)}) Tj`, 'ET'].join('\n');
              const ap2 = context.stream(content2, { Type:'XObject', Subtype:'Form', BBox:[0,0, w2, h2], Matrix:[1,0,0,1,0,0], Resources:{ Font:{ F1: helv.ref } } });
              const newWidget = context.obj({ Type:'Annot', Subtype:'Widget', Rect:[X1, Y1, X2, Y2], P: pg2.ref, AP:{ N: context.register(ap2) }, F: 4, Parent: parentFieldRef });
              const newWidgetRef = context.register(newWidget);
              try { let ann2 = pg2.node.lookupMaybe ? pg2.node.lookupMaybe(PDFName.of('Annots')) : null; if (!ann2) pg2.node.set(PDFName.of('Annots'), context.obj([ newWidgetRef ])); else if (ann2.push) ann2.push(newWidgetRef); } catch {}
            }
          }
        }
        pdfWithPlaceholder = await pdfDoc3.save({ useObjectStreams: false });
      } catch {}
    }
...
// (removed) /sign/pdf-resign-flatten-old alias
  ...
  try {
    ...
    const rectOverride = (req.body && Array.isArray(req.body.rect) && (req.body.rect.length === 4 || req.body.rect.length === 2))
      ? req.body.rect.map((n) => parseInt(n, 10))
      : null;
    const rectMode = (req.body && typeof req.body.rectMode === 'string') ? String(req.body.rectMode).toLowerCase() : 'top-left';
...
app.post('/sign/pdf-batch', requireAuth, async (req, res) => {
  try {
    ...
    const embedIntermediates = req.body && req.body.embedIntermediates !== undefined ? !!req.body.embedIntermediates : false;
    const signingTime = parseLocalTime(req.body && req.body.signingTime);
    // Accept either [x1,y1,x2,y2] (PDF coords) or [left, top] in 'top-left' rectMode
    const rectOverride = (req.body && Array.isArray(req.body.rect) && (req.body.rect.length === 4 || req.body.rect.length === 2))
      ? req.body.rect.map((n) => parseInt(n, 10))
      : null;
    const anchor = (req.body && typeof req.body.anchor === 'string') ? String(req.body.anchor).toLowerCase() : 'top-left';
    ...
    // Unified flow will handle key detection and signing per file to keep logic identical to /sign/pdf

    async function signOne(b64) {
      try {
        const inputBuf = Buffer.from(b64, 'base64');
        assertNoExistingSignature(inputBuf);
        // Reuse the unified flow used by /sign/pdf so coords, pages, and stamping behave identically
        const signedPdf = await signWithUnifiedFlow(inputBuf, req.body || {}, dll, pin);
        let outBuf;
        if (Buffer.isBuffer(signedPdf)) outBuf = signedPdf;
        else if (signedPdf instanceof Uint8Array) outBuf = Buffer.from(signedPdf);
        else if (signedPdf && signedPdf.buffer instanceof ArrayBuffer) outBuf = Buffer.from(new Uint8Array(signedPdf.buffer));
        else if (signedPdf && Array.isArray(signedPdf.data)) outBuf = Buffer.from(signedPdf.data);
        else outBuf = Buffer.from(signedPdf);
        return { ok: true, signedPdfBase64: outBuf.toString('base64') };
      } catch (e) {
