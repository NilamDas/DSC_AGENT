...
const { resolveSigningContextFromTimeServerOrBody, authorizeServerSignOrBypass } = require('./services/signingTimeService');
...
app.post('/sign/text', requireAuth, async (req, res) => {
  try {
    let signingContext;
    let authResult;
    const signRequestId = 'sign-text-' + Date.now() + '-' + crypto.randomBytes(3).toString('hex');

    console.log('[sign/text] resolving signing context...');
    try {
      const tsBody = buildTimeServerUserBody(req.body);
      signingContext = await resolveSigningContextFromTimeServerOrBody(tsBody, { parseLocalTime, timeServerClient });
      authResult = await authorizeServerSignOrBypass(tsBody, { timeServerClient }, signingContext);
    } catch (authErr) {
      const status = authErr && typeof authErr.status === 'number'
        ? ((authErr.status === 401 || authErr.status === 403) ? 401 : 502)
        : 500;
      return res.status(status).json({
        ok: false,
        message: 'Signing authorization failed.',
        reason: authErr && authErr.message ? String(authErr.message) : 'authorization_error',
      });
    }

    const signingTime2 = signingContext.signingTime;
    const authPassed = !!(authResult && authResult.ok === true && authResult.bypassed !== true);
    console.log('[sign/text][time-server-debug]', {
      requestId: signRequestId,
      contextSource: signingContext.source || 'unknown',
      tokenPresent: !!signingContext.token,
      authBypassed: !!(authResult && authResult.bypassed),
      authBypassReason: (authResult && authResult.reason) || null,
      tokenRefreshed: !!(authResult && authResult.refreshed),
      authOk: authPassed,
    });
    if (!authPassed) {
      return res.status(401).json({
        ok: false,
        message: 'JWT verification/authorization failed. Signing not performed.',
        reason: (authResult && authResult.reason) || 'authorization_failed',
      });
    }
...
app.post('/sign/pdf', requireAuth, async (req, res) => {
  try {
    ...
    console.log('[sign/pdf] resolving signing context...');
    try {
      // First resolve signing context (time, token info, etc.) and perform authorization .
      const tsBody = buildTimeServerUserBody(req.body);
      signingContext = await resolveSigningContextFromTimeServerOrBody(tsBody, { parseLocalTime, timeServerClient });

      // Then perform authorization (which may involve time server checks, token presence checks, etc.) and determine if signing can proceed.
      authResult = await authorizeServerSignOrBypass(tsBody, { timeServerClient }, signingContext);
    ...
    const signingTime2 = signingContext.signingTime;
    const authPassed = !!(authResult && authResult.ok === true && authResult.bypassed !== true);
    console.log('[sign/pdf][time-server-debug]', {
      requestId: signRequestId,
      contextSource: signingContext.source || 'unknown',
      tokenPresent: !!signingContext.token,
      authBypassed: !!(authResult && authResult.bypassed),
...
app.post('/sign/pdf-batch', requireAuth, async (req, res) => {
  try {
    ...
    console.log('[sign/pdf-batch] resolving signing context...');
    try {
      const tsBody = buildTimeServerUserBody(req.body);
      signingContext = await resolveSigningContextFromTimeServerOrBody(tsBody, { parseLocalTime, timeServerClient });
      authResult = await authorizeServerSignOrBypass(tsBody, { timeServerClient }, signingContext);
    ...
    const signingTime2 = signingContext.signingTime;
    const authPassed = !!(authResult && authResult.ok === true && authResult.bypassed !== true);
    console.log('[sign/pdf-batch][time-server-debug]', {
      requestId: signRequestId,
      contextSource: signingContext.source || 'unknown',
      tokenPresent: !!signingContext.token,
      authBypassed: !!(authResult && authResult.bypassed),
    ...
    const embedIntermediates = req.body && req.body.embedIntermediates !== undefined ? !!req.body.embedIntermediates : false;
    const signingTime = signingTime2;
    console.log('Batch signing time:', signingTime);
...
app.post('/sign/pdf-resign-flatten', requireAuth, async (req, res) => {
  ...
  try {
    ...
    console.log('[sign/pdf-resign-flatten] resolving signing context...');
    try {
      const tsBody = buildTimeServerUserBody(req.body);
      signingContext = await resolveSigningContextFromTimeServerOrBody(tsBody, { parseLocalTime, timeServerClient });
      authResult = await authorizeServerSignOrBypass(tsBody, { timeServerClient }, signingContext);
    ...
    const signingTime2 = signingContext.signingTime;
    const authPassed = !!(authResult && authResult.ok === true && authResult.bypassed !== true);
    console.log('[sign/pdf-resign-flatten][time-server-debug]', {
      requestId: signRequestId,
      contextSource: signingContext.source || 'unknown',
      tokenPresent: !!signingContext.token,
      authBypassed: !!(authResult && authResult.bypassed),
